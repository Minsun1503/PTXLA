...
def get_coordinates_from_user_selection(file_path: str, coord_save_path: str, cfg: config.Config):
    ...
    print("--> Sử dụng chế độ: PDF Gốc (Bỏ qua bước dò tìm biên)")
    warped_standard = cv2.resize(original_image, cfg.ImageProcessing.STANDARD_SIZE)

    # Để outside_image giống hệt warped_standard (để Phase 2 nhìn thấy toàn bộ tờ giấy)
    outside_image = warped_standard.copy()
    ...
    target_image_for_ocr = warped_standard
    p2_h, p2_w = target_image_for_ocr.shape[:2]
    p2_display_w = cfg.UI.DISPLAY_WIDTH
    p2_display_scale = p2_w / p2_display_w
    p2_display_h = int(p2_h / p2_display_scale)

    display_image_phase2 = cv2.resize(target_image_for_ocr, (p2_display_w, p2_display_h))
