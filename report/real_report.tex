...
% --- THÔNG TIN ---
\title{\textbf{BÁO CÁO ĐỒ ÁN CUỐI KÌ \\ MÔN HỌC: PHÂN TÍCH XỬ LÍ ẢNH} \\ HỆ THỐNG CHẤM THI VÀ SỐ HÓA THÔNG TIN TỰ ĐỘNG (SMARTGRADER)}
...
\begin{document}
	...
	\begin{figure}[H]
		\centering
		% Bạn nhớ tải ảnh sơ đồ EasyOCR về và lưu tên là easyocr_pipeline.png vào thư mục images nhé
		\includegraphics[height=\textheight, keepaspectratio]{images/easyocr_pipeline.png}
		\caption{Sơ đồ kiến trúc đường ống xử lý của EasyOCR: Từ phát hiện (Detection) đến nhận dạng (Recognition)}
	...
	\begin{figure}[H] 
		...
		\includegraphics[height=\textheight, keepaspectratio]{images/pipeline.png}
	...
	Để làm rõ sự tương tác giữa các thành phần trong kiến trúc MVC, sơ đồ tuần tự dưới đây mô tả chi tiết luồng trao đổi thông điệp khi hệ thống xử lý một lô ảnh đầu vào:
	...
	Quá trình này cho thấy sự phân tách rõ ràng: \texttt{Main Controller} điều phối luồng đi, \texttt{Processor} chịu trách nhiệm xử lý ảnh thô, trong khi \texttt{OMR/OCR Engines} thực hiện logic nghiệp vụ sâu và \texttt{Renderer} chỉ tập trung vào hiển thị kết quả.
	...
	Module này (\texttt{src/core/ocr\_engine.py}) tập trung vào nhiệm vụ quan trọng nhất là trích xuất và chuẩn hóa \textbf{Số báo danh (SBD)} của thí sinh. Do đặc thù của thư viện EasyOCR khi xử lý chữ viết tay tiếng Việt còn hạn chế, nhóm tập trung tối ưu hóa việc nhận dạng chuỗi số SBD để đảm bảo định danh chính xác bài thi.
	
	\subsection{Quy trình xử lý}
	\begin{enumerate}
		\item \textbf{Cắt vùng ảnh (ROI Segmentation):} Dựa trên toạ độ vùng SBD được định nghĩa trong \texttt{coordinates.json}, hệ thống cắt vùng ảnh chứa số báo danh.
		\item \textbf{Tiền xử lý OCR:} Sử dụng \texttt{Otsu's Binarization} kết hợp với việc chuyển đổi ảnh xám để làm nổi bật các chữ số trên nền giấy.
		\item \textbf{Nhận dạng:} Gọi thư viện EasyOCR để đọc dữ liệu thô từ vùng ảnh.
	\end{enumerate}
	
	\subsection{Hậu xử lý dữ liệu bằng Regex (Regular Expression)}
	Dữ liệu thô (Raw Text) từ OCR thường chứa nhiều nhiễu. Ví dụ: số "1" bị đọc nhầm thành chữ "l" hoặc "|", số "0" bị đọc thành chữ "O", hoặc xuất hiện các ký tự rác như dấu chấm, phẩy.
	
	Để khắc phục, hệ thống áp dụng bộ lọc Regex nghiêm ngặt chỉ giữ lại các ký tự số:
	
	\begin{lstlisting}[language=Python, caption={Hàm làm sạch Số báo danh bằng Regex}]
		def _post_process_text(self, key_name: str, raw_text: str) -> str:
		text = raw_text.strip()
		
		# Chi xu ly truong SBD (So Bao Danh)
		if "sbd" in key_name.lower():
		# Regex \D: Khop moi ky tu KHONG phai la so
		# Thay the ky tu rac bang chuoi rong
	...
	Kỹ thuật này đảm bảo kết quả cuối cùng của SBD luôn là một chuỗi số sạch, loại bỏ hoàn toàn các ký tự gây nhiễu.
	\section{Tinh chỉnh Mô hình Nhận dạng (OCR Fine-tuning)}
	
	Để khắc phục hạn chế của model gốc (thường nhận diện sai chữ viết tay tiếng Việt và các ký tự số bị viết tháu), nhóm đã thực hiện quy trình huấn luyện lại mô hình trên nền tảng Kaggle GPU.
	
	\subsection{Chuẩn bị dữ liệu (Data Preparation)}
	\begin{itemize}
		\item \textbf{Nguồn dữ liệu:} Sử dụng bộ dataset \textit{OCR\_handwritting\_HAT2023} (HuggingFace), bao gồm các mẫu chữ viết tay tiếng Việt đa dạng \cite{hat2023}.
		\item \textbf{Tiền xử lý:} Dữ liệu ảnh thô và nhãn (Label) được chuyển đổi sang định dạng \textbf{LMDB (Lightning Memory-Mapped Database)} để tối ưu hóa tốc độ đọc ghi (I/O) trong quá trình huấn luyện.
	\end{itemize}
	
	\subsection{Cấu hình Huấn luyện (Training Configuration)}
	Quá trình huấn luyện được thực hiện với các siêu tham số (Hyperparameters) sau:
	\begin{itemize}
		\item \textbf{Base Model:} \texttt{english\_g2.pth} (Pre-trained).
		\item \textbf{Architecture:} ResNet (Feature) + BiLSTM (Sequence) + CTC (Prediction).
		\item \textbf{Hidden Size / Output Channel:} 256 (Tối ưu hóa cho mô hình nhẹ).
		\item \textbf{Iterations:} 3000 vòng lặp.
		\item \textbf{Batch Size:} 64.
	\end{itemize}
	
	\subsection{Giải quyết vấn đề bất đồng bộ kiến trúc (Technical Challenges)}
	Trong quá trình Fine-tuning, nhóm gặp phải vấn đề \textit{"Size Mismatch"} (Lệch kích thước tensor) giữa lớp Prediction của model gốc (97 classes - tiếng Anh) và model mới (197 classes - tiếng Việt + dấu).
	
	\textbf{Giải pháp "Smart Loading":} Nhóm đã phát triển một thuật toán custom để load trọng số (weights), tự động lọc bỏ các lớp bị lệch kích thước và chỉ giữ lại các lớp Feature Extraction quan trọng.
	
	\begin{lstlisting}[language=Python, caption={Thuật toán Smart Loading xử lý lệch size tensor}]
		# Doan code xu ly viec load weights khi so luong class thay doi
		state_dict = torch.load(opt.saved_model)
		model_dict = model.state_dict()
		
		# Loc bo cac lop (layers) khong khop kich thuoc
		new_state_dict = {
			k: v for k, v in state_dict.items() 
			if k in model_dict and v.shape == model_dict[k].shape
		}
		
		# Load cac weights tuong thich, khoi tao lai cac weights bi lech
		model.load_state_dict(new_state_dict, strict=False)
	\end{lstlisting}
	\section{Cơ chế Nhận dạng Kép và Chiến lược Hợp nhất (Hybrid OCR Strategy)}
	
	Kết quả thực nghiệm cho thấy một sự phân hóa rõ rệt về hiệu năng giữa hai mô hình trên các loại dữ liệu khác nhau. Thay vì phụ thuộc hoàn toàn vào một mô hình, hệ thống áp dụng chiến lược \textbf{Hybrid Strategy} để tận dụng sở trường riêng biệt của từng engine.
	
	\subsection{Phân tích hiệu năng so sánh}
	Bảng so sánh thực tế (Hình \ref{fig:dual_mode_log}) cho thấy:
	\begin{itemize}
		\item \textbf{Đối với trường thông tin chữ in/Tiếng Anh (Tên trường, Lớp, Môn học):} Mô hình Gốc (Default) hoạt động vượt trội. Ví dụ: Chuỗi "HCMUS" được mô hình gốc nhận diện chính xác, trong khi mô hình Custom nhận diện sai thành "hmus" do xu hướng gán ghép về các âm tiết tiếng Việt.
		\item \textbf{Đối với trường số viết tay (SBD, Mã đề):} Mô hình Custom (Fine-tuned) thể hiện ưu thế tuyệt đối trong việc xử lý các nét chữ viết tay tự nhiên, dính nét hoặc đứt đoạn, nơi mà mô hình gốc thường bỏ sót hoặc nhận diện nhầm.
	\end{itemize}
	
	\subsection{Cơ chế quyết định (Decision Logic)}
	Dựa trên phân tích trên, hệ thống sử dụng thuật toán lựa chọn kết quả theo ngữ cảnh:
	\begin{itemize}
		\item Nếu vùng nhận dạng là \textbf{Số Báo Danh (SBD)} hoặc \textbf{Mã đề}: Ưu tiên kết quả từ \textit{Custom Engine}.
		\item Nếu vùng nhận dạng là \textbf{Thông tin hành chính (Tên, Lớp, Môn)}: Ưu tiên kết quả từ \textit{Default Engine}.
	\end{itemize}
	
	Chiến lược này giúp hệ thống đạt độ chính xác tổng thể tối ưu, khắc phục được điểm yếu "học tủ" (overfitting) của mô hình tự huấn luyện đối với các từ vựng tiếng Anh hoặc chữ in chuyên ngành.
	
	\begin{figure}[H]
		\centering
		% Thay bằng ảnh chụp màn hình log thực tế của bạn
		\includegraphics[width=0.95\linewidth]{images/log_dual_mode.png}
		\caption{Log so sánh thực tế cho thấy sự vượt trội của Model Gốc đối với các trường thông tin chữ in (HCMUS, 21TTH)}
	...
	Quá trình huấn luyện trên Kaggle (GPU T4 x2) cho thấy sự hội tụ nhanh chóng của mô hình nhờ áp dụng Transfer Learning.
	
	\begin{itemize}
		\item \textbf{Khởi tạo:} Loss ban đầu rất cao ($\approx 44.8$) do lớp Prediction được khởi tạo ngẫu nhiên.
		\item \textbf{Sau 200 vòng lặp:} Train Loss giảm mạnh xuống còn $\approx 4.1$. Model bắt đầu học được cấu trúc ký tự tiếng Việt.
		\item \textbf{Kết thúc (3000 vòng):} Model đạt trạng thái ổn định, có khả năng nhận diện tốt các nét chữ viết tay và dấu tiếng Việt.
	\end{itemize}
	
	\section{Kết quả Chấm thi và OCR}
	
	\subsection{Đánh giá độ chính xác OCR}
	So sánh hiệu năng giữa Model gốc (Base) và Model sau khi tinh chỉnh (Fine-tuned) trên tập test gồm các mẫu chữ viết tay khó:
	
	\begin{table}[H]
		\centering
		\begin{tabular}{|l|l|l|}
			\hline
			\textbf{Trường hợp} & \textbf{Model Gốc (English\_g2)} & \textbf{Model Fine-tuned (SmartGrader)} \\ \hline
			SBD viết tay chuẩn & Chính xác (95\%) & Chính xác (98\%) \\ \hline
			SBD viết tháu/dính & Sai (thường nhầm 1 thành l) & \textbf{Cải thiện rõ rệt (85\%)} \\ \hline
			Ký tự có dấu & Không nhận diện được & Nhận diện được (Ví dụ: ế, ỗ) \\ \hline
		\end{tabular}
		\caption{So sánh hiệu năng OCR trước và sau khi Fine-tune}
	...
	Module OCR được kiểm thử tập trung vào khả năng trích xuất Số báo danh (SBD). Kết quả thực nghiệm cho thấy vai trò quyết định của bước hậu xử lý (Post-processing) bằng Regex.
	
	Trong nhiều trường hợp, EasyOCR trả về kết quả có lẫn ký tự lạ do vết bẩn trên giấy hoặc nét chữ không rõ ràng. Tuy nhiên, sau khi qua lớp lọc Regex, hệ thống chỉ giữ lại các con số, đưa ra SBD chính xác.
	
	\begin{table}[h]
		\centering
		\begin{tabular}{|l|l|l|}
			\hline
			\textbf{Case Thử nghiệm} & \textbf{Kết quả Log (Sau Regex)} & \textbf{Đánh giá} \\ \hline
			Case 0 (Ảnh chuẩn) & + SBD: 150320 & \textbf{Chính xác} \\ \hline
			Case 1 (Ảnh chụp) & + SBD: 225401 & \textbf{Chính xác} \\ \hline
			Case 2 (Bóng đổ) & + SBD: 150320 & \textbf{Chính xác} \\ \hline
			Case 3 (Nghiêng) & + SBD: 3621?? & Không nhận đủ số (Do góc nghiêng) \\ \hline
			Case 5 (Dấu tích) & + SBD: ?32??? & Lỗi do viết tay/tô không chuẩn \\ \hline
		\end{tabular}
	...
	\textbf{Nhận xét:} 
	\begin{itemize}
		\item Đối với các ảnh có chất lượng tốt và trung bình (Case 0, 1, 2), hệ thống trích xuất SBD hoàn toàn chính xác.
		\item Đối với các trường hợp ảnh bị nghiêng quá nhiều (Case 3) hoặc thí sinh viết/tô không theo quy chuẩn (Case 5), OCR gặp khó khăn trong việc nhận diện đầy đủ các chữ số. Đây là hạn chế đã được dự báo trước của thư viện EasyOCR đối với dữ liệu chữ viết tay phức tạp.
	\end{itemize}
	...
	\begin{enumerate}
		...
		\item \textbf{Tính tĩnh của cấu trúc (Static Template):} Hệ thống hoạt động dựa trên cấu trúc tọa độ định sẵn. Điều này đồng nghĩa với việc nếu mẫu phiếu thi thay đổi bố cục (layout), người dùng cần phải cập nhật lại tham số trong module sinh đề (\texttt{generate\_sheet.py}) để hệ thống tính toán lại lưới tọa độ mới, thay vì khả năng tự động thích nghi (Adaptive Layout Analysis).
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item \textbf{Giới hạn của module OCR:} Hiện tại hệ thống chỉ tập trung xử lý tốt trường thông tin \textbf{Số báo danh} (dạng số). Khả năng nhận dạng chữ viết tay tiếng Việt (như Họ tên, Lớp) chưa được đưa vào báo cáo chính thức do độ chính xác của model EasyOCR gốc trên các dữ liệu này chưa đạt yêu cầu thực tế (thường xuyên bị lỗi dấu tiếng Việt và nhầm lẫn ký tự).
		
		\item \textbf{Tính tĩnh của cấu trúc (Static Template):} Hệ thống hoạt động dựa trên file cấu hình tọa độ cố định (\texttt{coordinates.json}). Điều này đồng nghĩa với việc nếu mẫu phiếu thi thay đổi bố cục (layout), người dùng cần phải cập nhật lại các tham số kích thước trong module sinh đề (generate\_sheet.py) để hệ thống tính toán lại lưới tọa độ mới, thay vì khả năng tự động thích nghi (Adaptive Layout Analysis).
	\end{enumerate}
	...
	Mở rộng tập dữ liệu huấn luyện: Tiếp tục huấn luyện mô hình trên các bộ dữ liệu chữ viết tay quy mô lớn hơn để cải thiện khả năng nhận dạng các trường thông tin chữ (Tên, Lớp) vốn đang có độ chính xác chưa cao.
